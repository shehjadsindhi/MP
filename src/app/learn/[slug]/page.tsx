...
export async function generateMetadata({ params }: { params: { slug: string } }) {
  ...
  return {
    ...
    description: article.excerpt || "Galaxy AI Learning Guide and Tutorial",
  };
}
...
export default async function ArticleDetailPage({
  ...
  const relatedArticles = categoryArticles.filter((a) => a.id !== article.id && a.slug !== article.slug).slice(0, 2);
  ...
  const authorInitial = article.author ? article.author.charAt(0).toUpperCase() : "G";
  const authorName = article.author || "Galaxy AI Research";

  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-12 pb-24">
      {/* Breadcrumb */}
      <div className="flex items-center gap-2 text-xs text-gray-400">
        <Link href="/learn" className="hover:text-white flex items-center gap-1 transition-colors">
          <ArrowLeft className="w-3.5 h-3.5" /> All Guides
        </Link>
        <span>/</span>
        <span className="text-gray-500">{article.category || "Guide"}</span>
        <span>/</span>
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-3 text-xs">
          <span className="px-3 py-1 rounded-full bg-cyan-950/60 border border-cyan-500/30 text-galaxy-cyan font-bold uppercase">
            {article.category || "AI Guide"}
          </span>
          <span className="text-gray-400 flex items-center gap-1">
            <Clock className="w-3.5 h-3.5 text-galaxy-cyan" /> {article.readTime || "5 min read"}
          </span>
      ...
      <div className="rounded-3xl bg-galaxy-900 border border-slate-800 p-8 sm:p-12 flex items-center justify-center shadow-2xl h-80 overflow-hidden">
        <img
          src={article.image || "/images/nova_ultra.jpg"}
          alt={article.title}
