    ...
    <section id="persona-section" className="py-20 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-cyan-950/80 border border-cyan-500/40 text-galaxy-cyan text-xs font-bold uppercase tracking-wider mb-4 shadow-galaxy-cyan backdrop-blur-md">
            <Sparkles className="w-4 h-4 text-galaxy-cyan animate-pulse" /> Galaxy AI Persona Matcher
          </div>
          ...
          {PERSONAS.map((persona) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 px-5 py-3 rounded-2xl text-xs sm:text-sm font-bold transition-all duration-300 ${
                  isSelected
                    ? "bg-gradient-to-r from-galaxy-cyan via-cyan-400 to-blue-600 text-galaxy-950 shadow-galaxy-cyan scale-105 shimmer-btn"
        ...
        <div className="rounded-3xl bg-galaxy-900/80 border border-cyan-500/30 shadow-2xl p-6 sm:p-10 backdrop-blur-xl relative">
          {/* Header Banner */}
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-8 border-b border-slate-800">
            <div>
              <div className="flex items-center gap-3 mb-1">
                ...
                <span className="px-2.5 py-0.5 rounded-full bg-emerald-950/80 border border-emerald-500/40 text-emerald-400 text-[11px] font-extrabold flex items-center gap-1 shadow-sm">
                  <Sparkles className="w-3 h-3" /> {matchScores[activePersona.id] || 98}% AI Synergy Match
                </span>
              </div>
              <h3 className="text-2xl font-extrabold text-white">{activePersona.title}</h3>
              <p className="text-sm text-gray-300 mt-1 max-w-2xl leading-relaxed">{activePersona.tagline}</p>
            </div>

            <button
              onClick={handleSaveToAccount}
              className={`inline-flex items-center gap-2 px-4 py-2.5 rounded-xl border text-xs font-bold transition-all self-start md:self-auto ${
                savedSuccess
            ...
            >
              ...
              {savedSuccess ? "Saved to Account!" : "Save Preference to Account"}
            </button>
          </div>
